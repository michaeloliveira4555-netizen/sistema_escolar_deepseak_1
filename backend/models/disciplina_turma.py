...
class DisciplinaTurma(db.Model):
    ...
    # --- ALTERAÇÃO AQUI ---
    # Agora teremos dois campos para instrutores. O primeiro é obrigatório, o segundo é opcional.
    instrutor_id_1: Mapped[int] = mapped_column(db.ForeignKey('instrutores.id'), nullable=False)
    ...
    instrutor_1: Mapped["Instrutor"] = relationship(foreign_keys=[instrutor_id_1])
    instrutor_2: Mapped[t.Optional["Instrutor"]] = relationship(foreign_keys=[instrutor_id_2])

    def __init__(self, pelotao: str, disciplina_id: int, instrutor_id_1: int, instrutor_id_2: t.Optional[int] = None, **kw: t.Any) -> None:
