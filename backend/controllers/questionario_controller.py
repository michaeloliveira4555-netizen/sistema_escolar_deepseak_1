...
def novo_questionario():
    if request.method == 'POST':
        ...
        db.session.flush()  # Para obter o ID do novo questionário
...
def resultado_questionario(questionario_id):
    ...
    respostas = db.session.scalars(select(Resposta).where(Resposta.questionario_id == questionario_id)).all()
    ...
    return render_template('questionario/resultado.html', questionario=questionario, respostas=respostas, dados_graficos=dados_graficos)
...
def realizar_questionario():
    ...
    questionarios = db.session.scalars(select(Questionario).order_by(Questionario.titulo)).all()
    alunos = db.session.scalars(select(User).where(User.role == 'aluno').order_by(User.nome_completo)).all()
    instrutores = db.session.scalars(select(User).where(User.role == 'instrutor').order_by(User.nome_completo)).all()
    
    return render_template('questionario/realizar.html', questionarios=questionarios, alunos=alunos, instrutores=instrutores)
...
def get_perguntas(questionario_id):
    ...
    for p in questionario.perguntas:
        ...
        perguntas_list.append({'id': p.id, 'texto': p.texto, 'opcoes': opcoes_list})
